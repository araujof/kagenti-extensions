...
// Package main is the authbridge-cpex binary: proxy-sidecar mode (HTTP
// forward + reverse proxies) with the cpex plugin compiled in. The
// cpex plugin embeds the ContextForge Plugin Extensibility runtime
// (Rust core + bundled APL governance layer, linked via CGO from
// libcpex_ffi.a) as a first-class AuthBridge plugin; sub-plugins
// surface as their own Invocation rows so abctl renders the chain
// inline.
//
// This binary is hardcoded to proxy-sidecar mode; YAML configs that
// specify a different mode are rejected at boot. For the standard
// no-CPEX deployments use cmd/authbridge-proxy (default), or
// cmd/authbridge-envoy / cmd/authbridge-lite for the corresponding
// shapes.
//
// Build constraints. The `cpex` build tag gates the cpex plugin
// import + manager factory wiring — without the tag the binary won't
// know about the plugin, which mirrors the in-tree pattern used by
// the other authbridge-* binaries. CGO is ENABLED at build time so
// the linker pulls in libcpex_ffi.a; the matching realManager is
// behind `cpex && cgo` and adapts cpex.PluginManager to the plugin's
// Manager interface.
package main
...
import (
	...
	// Plugins. Mirrors authbridge-proxy plus the cpex plugin (whose
	// init() registers under the name "cpex"). Manager factory is
	// installed below in main() before any pipeline is built.
...
func main() {
	...
	// Install the CPEX manager factory BEFORE any pipeline is built.
	// The plugin's Configure constructs a manager via this factory;
	// without it Configure fails loudly. newRealManager comes from
	// realmanager.go (cpex && cgo); a CGO-disabled build picks up the
	// stub from realmanager_nocgo.go that returns a clear "binary
	// built without CGO" error.
	cpexplugin.SetManagerFactory(newRealManager)
